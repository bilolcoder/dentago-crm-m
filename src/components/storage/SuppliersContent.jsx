...
const SuppliersContent = () => {
    ...
    const handleSave = () => {
        if (!formData.name) return console.log("Nomini kiriting");
                    ...
                    <table className="w-full min-w-[1100px]">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                <th className="px-6 py-5 text-left text-xs font-black text-slate-500 uppercase">Kompaniya / ID</th>
                                <th className="px-6 py-5 text-center text-xs font-black text-slate-500 uppercase">Aloqa Ma'lumotlari</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredSuppliers.length > 0 ? (
                                filteredSuppliers.map((supplier) => (
                                    <tr key={supplier.id} className="hover:bg-[#00BCE4]/[0.03]">
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 bg-gray-50 rounded-lg flex items-center justify-center text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-500 transition-all border border-gray-100">
                                                    <Building2 size={20} strokeWidth={2.5} />
                                                </div>
                                                <div>
                                                    <div className="text-sm font-bold text-slate-800 uppercase tracking-tight italic group-hover:text-[#00BCE4] transition-colors cursor-pointer">{supplier.name}</div>
                                                    <div className="text-xs text-slate-500 font-mono tracking-widest mt-0.5">#{String(supplier.id).slice(-4)}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col items-center">
                                                <div className="inline-flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-lg border border-gray-100 group-hover:border-blue-100 transition-all shadow-sm">
                                                    <Phone size={13} className="text-[#00BCE4]" strokeWidth={3} />
                                                    <span className="text-xs text-slate-600 tracking-wider">{supplier.phone || '—'}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
